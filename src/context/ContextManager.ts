...
import { MiMoCodeSettings, MIMOCODE_VIEW_TYPE } from "../types";
...
type ContextManagerDeps = {
  app: App;
  settings: MiMoCodeSettings;
  client: MiMoCodeClient;
...
export class ContextManager {
  ...
  private getLeafForRefresh(): WorkspaceLeaf | null {
    ...
    if (activeLeaf?.view.getViewType() === MIMOCODE_VIEW_TYPE) {
      return activeLeaf;
    }
  ...
  private getVisibleSidebarLeaf(): WorkspaceLeaf | null {
    const leaves = this.app.workspace.getLeavesOfType(MIMOCODE_VIEW_TYPE);
    if (leaves.length === 0) {
      return null;
    }
  ...
  async refreshContextForView(view: MiMoCodeView): Promise<void> {
    if (!this.settings.injectWorkspaceContext) {
      return;
    }
  ...
  private async refreshContext(leaf: WorkspaceLeaf): Promise<void> {
    ...
    const view = leaf.view instanceof MiMoCodeView ? leaf.view : null;
